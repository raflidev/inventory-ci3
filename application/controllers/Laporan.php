...
class Laporan extends CI_Controller
{
  ...
  public function proses()
  {
    ...
    if ($post['jenis_laporan'] == "Persediaan Barang") {
      if ($post['tgl_mulai'] == '' || $post['tgl_akhir'] == '') {
        if ($post['kategori'] == "semua") {
          ...
          $data['data'] = $this->db->query("SELECT * from persediaan inner join barang on persediaan.id_barang=barang.id_barang where barang.kategori = '" . $post['kategori'] . "'")->result_array();
        }
      } else {
        $from = date('Y-m-d', strtotime($post['tgl_mulai']));
        $to = date('Y-m-d', strtotime($post['tgl_akhir']));
        ...
        if ($post['kategori'] == "semua") {
          if ($from == $to) {
            $data['data'] = $this->db->query("SELECT * from persediaan inner join barang on persediaan.id_barang=barang.id_barang where persediaan.tanggal = '$from'")->result_array();
          } else {
            $data['data'] = $this->db->query("SELECT * from persediaan inner join barang on persediaan.id_barang=barang.id_barang where persediaan.tanggal between '$from' and '$to'")->result_array();
          }
        } else {
          if ($from == $to) {
            $data['data'] = $this->db->query("SELECT * from persediaan inner join barang on persediaan.id_barang=barang.id_barang where barang.kategori = '" . $post['kategori'] . "' and persediaan.tanggal = '$from'")->result_array();
          } else {
            $data['data'] = $this->db->query("SELECT * from persediaan inner join barang on persediaan.id_barang=barang.id_barang where barang.kategori = '" . $post['kategori'] . "' and persediaan.tanggal between '$from' and '$to'")->result_array();
          }
        }
      }
      $data['title'] = 'Laporan Daftar Persediaan Barang';
      $this->load->library('pdf');
      // $this->load->view('laporan/laporan_persediaan_barang', $data);
    ...
    if ($post['jenis_laporan'] == "Barang Masuk") {
      ...
      $this->load->library('pdf');
      $this->pdf->load_view('laporan/laporan_barang_masuk', $data);
    }
    ...
    if ($post['jenis_laporan'] == "Barang Keluar") {
      ...
      $this->load->library('pdf');
      $this->pdf->load_view('laporan/laporan_barang_keluar', $data);
    }
    ...
    if ($post['jenis_laporan'] == "Permintaan Barang") {
      ...
      $this->load->library('pdf');
      $this->pdf->load_view('laporan/laporan_permintaan_barang', $data);
    }
  }
}
